...
export class UsersService {
  ...
  constructor(private readonly cloudinaryService: CloudinaryService) {
    this.prisma = getPrismaClient() as PrismaClient;
    if (!this.prisma?.user) {
    ...
    try {
      ...
      const user = await this.prisma.user.create({
        data: {
          ...
          // Add projectId if you want to assign a project on creation
  ...
  async uploadProfileImage(
    ...
    try {
      ...
      const uploadResult = await this.cloudinaryService.uploadUserProfileImage(
        file,
        id,
      );
